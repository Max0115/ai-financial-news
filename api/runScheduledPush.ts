...
async function getFinancialCalendar(ai: GoogleGenAI) {
    ...
    const prompt = `今天是 ${todayStr}。請提供未來一週內（從今天開始）全球最重要的財經事件日曆。請包含日期（YYYY-MM-DD）、時間（HH:MM，24小時制，台灣時間 UTC+8）、國家/地區的 ISO 3166-1 alpha-2 代碼、事件的繁體中文名稱和重要性（High, Medium, Low）。以 JSON 格式回傳。`;
